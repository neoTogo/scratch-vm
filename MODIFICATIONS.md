...
| `src/extensions/scratch3_face_sensing/index.js` | One local change to otherwise-verbatim upstream code: `solutionPath` made document-relative. Upstream hardcodes `/chunks/mediapipe/face_detection`, which assumes Scratch is served from the site root; this platform serves it under `/scratch/`, so the absolute path missed and fell through to the CDN fallback. |
